...
Persistent Owner dispatch is disabled for M12. This document and
`coordination/OWNER_DISPATCH_MEMORY.yaml` retain historical Owner-channel
semantics only. New M12 work uses prompt-scoped task-agent dispatch with the
...
recorded under the M12 ignored evidence root and governed by
...
An active M12 child dispatch is invalid unless it explicitly records
