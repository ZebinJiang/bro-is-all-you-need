...
| LeRobot | `1396b9fab7aecddd10006c33c47a487ffdcb54b4` (`v0.5.1`, v3.0 dataset target) | Apache-2.0 | upstream schema/docs metadata only | Generate tiny LeRobot v3-like directory evidence with metadata/data relationships, without depending on the full package. | `genesisvla/testing/fixtures/**`, `tests/dataloader/test_tiny_fixtures.py` |
| PyArrow | `pypi:pyarrow==18.1.0` | Apache-2.0 | `pyarrow.parquet` write/read API | Use a pinned test/quality-only parquet backend for generated fixture evidence. | `genesisvla/testing/fixtures/**`, `tests/dataloader/**` |
