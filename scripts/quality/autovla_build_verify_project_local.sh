...
if [[ ! -x "$TOOL_PY" ]]; then
  ...
  echo "run make autovla-check-bootstrap before autovla-build-check"
  exit 127
fi
...
genesis_path = Path(autovla.__file__).resolve()
...
if not any(genesis_path.is_relative_to(path) for path in site_packages):
    raise SystemExit(f"autovla import is not from clean venv site-packages: {genesis_path}")
...
print(f"PASS import_autovla: {genesis_path}")
PY
