...
def main() -> int:
    ...
    for raw in args.paths:
        ...
        candidates.append({
            ...
            "exists": path.exists(),
            "type": "directory" if path.is_dir() else "file" if path.is_file() else "other/missing",
            "size_bytes": path_size(path),
            "contains": preview(path),
            "inferred_role": infer_role(path),
            "why_may_be_safe_to_delete": "Manager must fill this after audit; this script only inventories candidates.",
            "risk": "unknown until Manager audit",
            "recovery_option": "restore from git, backup, or regenerate if applicable; Manager must verify before asking user confirmation",
        })

    proposal = {"run_id": args.run_id, "candidates": candidates, "deletes_nothing": True}
    ...
    lines = ["# Cleanup Proposal", "", f"Run id: `{args.run_id}`", "", "This is a read-only proposal. It deletes nothing.", ""]
